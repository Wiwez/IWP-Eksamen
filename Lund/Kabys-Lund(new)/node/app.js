...
function isValidOrder(order) {
  return order.name && order.strength && order.milk && order.orderTime && typeof order.isReady === 'boolean' && order.orderID;
}
/* *********************************************************************
        ...
        case "test": // endpoint POST /test
          ...
          .then(data => {
            ...
            if (isValidOrder(validOrder)) {
              orderQueue.push(validOrder);
              console.log(orderQueue);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify(orderQueue)); // Send the updated orderQueue back to the client
            } else {
