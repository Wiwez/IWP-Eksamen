...
function validateCoffeeFormData(coffeeData){let name; let nameLen;
  let milk;
  let strength;
  try{
    ...
    if ((nameLen>=minNameLength) && (nameLen<=maxNameLength) && (minStrength <= strength) && (strength <= maxStrength)
        ((milk===nomilk) || (milk===oatmilk) || (milk===cowmilk))) {
        let validOrderData = {name: name, strength: strength, milk: milk};
        console.log("Validated: "); console.log(validOrderData);
...
let sampleCoffeeOrder={name:"Anders", strength: 4, milk: nomilk, orderID: orderID, orderTime: new Date(), isReady:false};
