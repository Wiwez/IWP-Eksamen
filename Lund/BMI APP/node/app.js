...
function validateBMIRecordForm(bmiData){
  ...
  if(bmiData.has("userName") && bmiData.has("weight") && bmiData.has("height") && bmiData.has("age")){
    let userName=String(bmiData.get("userName"));
    userName=validateUserName(userName);
    let sex = bmiData.get("sex");
    console.log(sex);
    sex = validateSex(sex);
...
function validateSex(sex){
  if(sex===femaleSex || sex===maleSex || sex=== indifferentSex){
     console.log("Validated sex");
    return sex;
  }
  throw(new Error(ValidationError)); //Exceptions treated in a future class
}
...
function renderHTMLBMIUpdatePage(bmiStatus){
  let page=renderHTMLHdr("BMI Status",["/c  ss/simple.css"]);
  page+=`<body><section>
<h1> BMI Status of ${bmiStatus.userName} </h1>
<output>
Your BMI is ${bmiStatus.bmi}. Since last, it has changed ${bmiStatus.delta}. 
</output>
${renderHTMLBMILinks("/", "Back to main page")}
${renderHTMLBMILinks("html/BMIHelp.html", "Get Help")}
${renderHTMLBMILinks(`bmi-records?userName=${bmiStatus.userName}`, `See status page for ${bmiStatus.userName}`)}
</section></body>`;
 return page;
}
...
function renderHTMLBMILinks(url, text){
